...
// Look a product up by id and send the WhatsApp enquiry. Used by delegated
// click handlers so we never have to embed product data inside HTML attributes.
function enquireProductById(id) {
  const p = PRODUCTS.find(x => String(x.id) === String(id));
  if (p) enquireProduct(p);
}
...
function showToast(message, icon = 'check') {
  ...
  t.className = 'toast';
  t.innerHTML = `<span>${icon === 'redirect' ? '↗' : '✓'}</span><span>${escapeHtml(message)}</span>`;
  c.appendChild(t);
...
// Single delegated listener for every "Enquire on WhatsApp" button rendered
// anywhere on the site (shop grid, related grid, featured grid).
document.addEventListener('click', (e) => {
  const btn = e.target.closest('[data-enquire-id]');
  if (btn) enquireProductById(btn.dataset.enquireId);
});

const CATEGORY_IDS = ['living-room', 'bedroom', 'dining-room', 'office', 'storage'];
...
function buildGallery(p) {
  const images = [p.image_url, p.image_2, p.image_3, p.image_4].filter(Boolean);
  if (!images.length) images.push('images/products/placeholder.jpg');

  const main = document.getElementById('main-product-img');
  if (main) { main.src = images[0]; main.alt = p.name || 'Product image'; }

  const thumbList = document.getElementById('thumb-list');
  if (!thumbList) return;
  if (images.length <= 1) { thumbList.style.display = 'none'; return; }
  thumbList.style.display = '';
  thumbList.innerHTML = images.map((src, i) => `
    <div class="thumb-item${i === 0 ? ' active' : ''}">
      <img src="${escapeHtml(src)}" alt="${escapeHtml(p.name || 'Product')} — view ${i + 1}"
           onerror="this.parentElement.style.background='var(--bg-alt)'">
    </div>
  `).join('');
  thumbList.querySelectorAll('.thumb-item').forEach(t => t.addEventListener('click', () => {
    thumbList.querySelectorAll('.thumb-item').forEach(x => x.classList.remove('active'));
...
async function initProductPage() {
  ...
  const infoWrap = document.getElementById('product-extra-info');
  const infoBody = document.getElementById('product-extra-info-body');
  if (infoWrap && infoBody) {
    if (p.product_info && p.product_info.trim()) {
      infoBody.textContent = p.product_info;
